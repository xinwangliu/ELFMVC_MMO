function [H_normalized,gamma,obj,KA] = MKKM_IK(K,S,cluster_count,qnorm,algorithm_choose)
...
%     resH(iter,:) = myNMIACC(H,Y,cluster_count);
   %% updata base kernels
    ...
    %% KC  = mycombFun(KA,gamma.^qnorm);
    KC  = mycombFun(KA,gamma.^qnorm);
    if iter>2 && (abs((obj(iter-1)-obj(iter))/(obj(iter-1)))<1e-4 || iter>50)
        flag =0;
    end
%     if iter>2 && (iter>100)
%         flag =0;
